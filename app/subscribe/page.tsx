...
const planTypeTabs = ["Individual", "Group", "Gift"];
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white py-12">
      <div className="w-full max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
        <h2 className="text-2xl font-semibold text-center mb-8">Choose a subscription plan</h2>
        <div className="flex justify-center gap-4 mb-8">
          {planTypeTabs.map(tab => (
            <button
              key={tab}
              type="button"
              className={`px-4 py-2 rounded font-medium border transition-colors ${selectedTab === tab ? "bg-gray-100 border-gray-400" : "bg-white border-gray-200 text-gray-500"}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {plans.map((plan, idx) => (
            <label
              key={plan.key}
              className={`flex flex-col border rounded-lg p-4 cursor-pointer transition-all ${selectedPlan === plan.key ? "border-green-600 shadow-md bg-green-50" : "border-gray-200 bg-white"}`}
            >
              <input
                type="radio"
                name="plan"
              ...
              <div className="flex items-center justify-between mb-2">
                <span className={`font-semibold ${selectedPlan === plan.key ? "text-green-700" : "text-gray-900"}`}>{plan.name}</span>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-lg tracking-widest">**** **** **** 3171</span>
        </div>
        <button className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded transition">Subscribe</button>
